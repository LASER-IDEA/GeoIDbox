...
This protocol is fundamentally different from random data splits used in prior work (e.g., BiG-C~\cite{narayanan2022enhanced}), where samples from the same sensor can appear in both training and test sets, leading to optimistic estimates. Our LOSO protocol reveals that conventional ML methods (RF, XGBoost) achieve only moderate improvement (28--30\,m vs.\ 36.96\,m baseline) and degrade 2.4--2.8$\times$ from their random-split performance, confirming that true zero-shot generalization is a genuinely hard problem that our $P_{\text{bias}}$ formulation specifically addresses.
...
We appreciate this suggestion and have substantially broadened our comparative evaluation. In the revised manuscript, we now provide comparisons at three levels:

\textbf{Level 1 --- Baseline Comparisons (Table~II):}
We retain our comparison with classical geostatistical (IDW, Ordinary Kriging) and machine learning methods (Random Forest, XGBoost), and have added four deep learning baselines (Plain MLP, SIREN, Hash-SIREN, TabNet). Under strict LOSO evaluation, RF (28.75\,m) and XGBoost (30.35\,m) achieve only moderate ${\sim}20\%$ improvement over the physics baseline (36.96\,m). More tellingly, these same methods achieve 10--13\,m MAE under random splits, revealing a \textbf{2.4--2.8$\times$ degradation} when evaluated on truly unseen hardware. This demonstrates that standard data-driven methods partially memorize sensor-specific hardware fingerprints, which inflate random-split results but fail under operationally meaningful evaluation.
