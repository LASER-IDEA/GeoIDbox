...
All baselines operate on the same physics-derived pressure bias field. For IDW and Kriging, the spatial interpolation is applied directly to the bias values at sensor locations. For RF and XGBoost, the models are trained to predict the pressure bias from geographic features (latitude, longitude, altitude) and environmental parameters (temperature, humidity). Under strict LOSO evaluation, the test sensor's data is completely excluded from training.
...
IDW~\cite{shepard1968two} & 37.50 & 3.93 & $-$1.5\% \\
Ordinary Kriging~\cite{cressie1993statistics} & 37.35 & 4.35 & $-$1.1\% \\
\midrule
\multicolumn{4}{c}{\textit{Traditional Machine Learning}} \\
Random Forest~\cite{breiman2001random} & 64.82 & 8.68 & $-$75.4\% \\
XGBoost~\cite{chen2016xgboost} & 65.09 & 7.96 & $-$76.1\% \\
\midrule
\multicolumn{4}{c}{\textit{Deep Learning Methods}} \\
Plain MLP (ReLU, $\Delta h$) & 47.05 & 47.13 & $-$27.3\% \\
SIREN MLP ($\Delta h$) & 22.21 & 18.39 & 39.9\% \\
Hash-SIREN ($\delta P$, no $P_{\text{bias}}$) & 20.83 & 7.89 & 43.6\% \\
TabNet~\cite{arik2021tabnet} & 59.41 & 9.44 & $-$60.8\% \\
\midrule
...
\begin{itemize}
    \item \textbf{The Inadequacy of Geostatistical Interpolation}: Classical spatial methods like IDW and Kriging achieve MAEs of 37.50 m and 37.35 m, respectively, which matches the uncalibrated Physics Baseline (36.96 m). This proves that spatial interpolation of the pressure bias field from neighboring sensors provides no meaningful metrological correction. In heterogeneous IoT networks, the primary error source is the random, per-sensor hardware offset, which overshadows geographic pressure variations.
    
    \item \textbf{Deep Learning Alone Does Not Solve Zero-Shot Transfer}: Even modern deep learning methods fail under strict LOSO evaluation. TabNet~\cite{arik2021tabnet}, a state-of-the-art attention-based tabular architecture, achieves only 59.41\,m MAE---worse than the physics baseline. A standard MLP (47.05\,m) similarly fails to generalize. SIREN~\cite{sitzmann2020implicit} (22.21\,m) and Hash-SIREN without $P_{\text{bias}}$ (20.83\,m) perform better but remain far from our full model. This hierarchy reveals that neither advanced neural architectures nor spatial hash encoding alone can decouple sensor-specific hardware biases; the physics-derived $P_{\text{bias}}$ feature is essential.
    
    \item \textbf{Severe Degradation of ML Regressors in Zero-Shot Transfer}: Under the strict LOSO evaluation, conventional supervised regressors exhibit severe spatial overfitting. Random Forest (64.82 m) and XGBoost (65.09 m) perform substantially worse than the raw physics baseline. Because the \emph{pressure\_bias} is dominated by hardware-specific calibration offsets that are not recoverable from geographic features alone, tree-based models erroneously memorize the hardware ``fingerprints'' of the training sensors. When evaluated on a completely unseen, uncalibrated device, they incorrectly apply these memorized offsets, leading to severe measurement degradation. This confirms that zero-shot altitude estimation is fundamentally an extrapolation problem that standard feature regression cannot solve.
