...
\midrule
Random Forest \cite{breiman2001random} & Data-driven & 28.75 m$^{\dagger}$ & No \\
XGBoost \cite{chen2016xgboost} & Data-driven & 30.35 m$^{\dagger}$ & No \\
Deep Learning \cite{chen2022deep} & Data-driven & 22--47 m$^{\dagger}$ & No \\
\midrule
\textbf{PINF (Ours)} & \textbf{Physics-Neural} & \textbf{3.55 m}$^{\dagger}$ & \textbf{Verified (LOSO)} \\
\bottomrule
\multicolumn{4}{l}{\footnotesize $^{\dagger}$Our 8-fold LOSO evaluation. Other accuracies from original publications.}
\end{tabular}
\end{table}
