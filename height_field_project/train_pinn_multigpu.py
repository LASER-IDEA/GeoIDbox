"""
Multi-GPU training script for PINN.
    ...
    for step, batch in enumerate(loader):
        ...
        with autocast('cuda', enabled=use_amp):
            ...
            if lambda_hydro > 0:
                ...
                loss_hydro = torch.mean(hydro_residual ** 2) / grad_accum_steps
...
def main(args: argparse.Namespace) -> None:
    ...
    # Physics baseline (convert_to_hae=False because GNSS altitude is already MSL)
    print("Computing physics baseline...")
    df, phys_params = compute_physics_baseline(
        df, 
        p_ref=args.p_ref, 
        t_ref_method="mean", 
        p_ref_method=args.p_ref_method,
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=args.restart_epochs, T_mult=2)
    ...
    for epoch in range(args.epochs):
        train_loss, _ = train_epoch(
            ...
            use_amp=args.use_amp, grad_accum_steps=args.grad_accum_steps
        )
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            print(f"Epoch {epoch+1:04d} | Train: {train_loss:.4f} | Val MAE: {val_metrics['mae']:.3f}m | Improvement: {val_metrics['improvement']:.1f}%")
        
        if run:
            wandb.log({'epoch': epoch + 1, 'train_loss': train_loss, 'val_mae': val_metrics['mae'], 'val_improvement': val_metrics['improvement']})
        ...
        if val_metrics['mae'] < best_val_mae - 1e-4:
            ...
            # Convert BarometricParams to dict for safe loading
            save_dict_safe = save_dict.copy()
            if hasattr(save_dict_safe['phys_params'], '__dict__'):
                save_dict_safe['phys_params'] = save_dict_safe['phys_params'].__dict__
            torch.save(save_dict_safe, os.path.join(args.artifacts_dir, "model.pt"))
        else:
            patience_counter += 1
    ...
    checkpoint = torch.load(os.path.join(args.artifacts_dir, "model.pt"), weights_only=False)
    ...
    if run:
        wandb.log({'test_mae': test_metrics['mae'], 'test_rmse': test_metrics['rmse'], 'test_improvement': test_metrics['improvement']})
        wandb.finish()
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train PINN with multi-GPU support")
    ...
    # Physics
    p.add_argument("--p_ref", type=float, default=None, help="Reference pressure (Pa). If None, auto-estimate from data")
    p.add_argument("--p_ref_method", type=str, default="auto", choices=["auto", "isa", "lowest_altitude", "regression"])
    ...
    p.add_argument("--wandb_run_name", type=str, default="pinn_v1")
