    ...
    for fold, held_out_sensor in enumerate(sensors):
        ...
        try:
            ...
            if len(coords_train_full) > max_idw_points:
                indices = np.random.choice(
                    len(coords_train_full), max_idw_points, replace=False
                )
            ...
            rmse_idw = np.sqrt(
                np.mean((h_pred_idw - test_df["avg_altitude"].values) ** 2)
            )

            fold_results["IDW_MAE"] = mae_idw
            fold_results["IDW_RMSE"] = rmse_idw
            print(
                f"    IDW - MAE: {mae_idw:.3f} m, RMSE: {rmse_idw:.3f} m ({time.time() - t0:.2f}s)"
            )
        ...
        try:
            ...
            if len(coords_train_full) > max_krig_points:
                indices = np.random.choice(
                    len(coords_train_full), max_krig_points, replace=False
                )
            ...
            rmse_krig = np.sqrt(
                np.mean((h_pred_krig - test_df["avg_altitude"].values) ** 2)
            )

            fold_results["Kriging_MAE"] = mae_krig
            fold_results["Kriging_RMSE"] = rmse_krig
            print(
                f"    Kriging - MAE: {mae_krig:.3f} m, RMSE: {rmse_krig:.3f} m ({time.time() - t0:.2f}s)"
            )
        except Exception as e:
        ...
        try:
            ...
            rmse_rf = np.sqrt(
                np.mean((h_pred_rf - test_df["avg_altitude"].values) ** 2)
            )

            fold_results["RF_MAE"] = mae_rf
            fold_results["RF_RMSE"] = rmse_rf
            print(
                f"    Random Forest - MAE: {mae_rf:.3f} m, RMSE: {rmse_rf:.3f} m ({time.time() - t0:.2f}s)"
            )
        except Exception as e:
        ...
        try:
            ...
            rmse_xgb = np.sqrt(
                np.mean((h_pred_xgb - test_df["avg_altitude"].values) ** 2)
            )

            fold_results["XGB_MAE"] = mae_xgb
            fold_results["XGB_RMSE"] = rmse_xgb
            print(
                f"    XGBoost - MAE: {mae_xgb:.3f} m, RMSE: {rmse_xgb:.3f} m ({time.time() - t0:.2f}s)"
            )
        except Exception as e:
