...
def plot_baseline_comparison(
    ...
    dl_csv="experiments/05_dl_baselines/results.csv",
):
    ...
    if os.path.exists(dl_csv):
        ...
        dl_methods = {
            "MLP": ("Plain\nMLP", PALETTE["yellow"]),
            "SIREN": ("SIREN\nMLP", PALETTE["light_blue"]),
            "HashSIREN": ("Hash-\nSIREN", "#8B6914"),
            "TabNet": ("TabNet", "#A0522D"),
        }
        for method_key, (label, color) in dl_methods.items():
            col = f"{method_key}_MAE"
            if col in dl_df.columns:
                labels.append(label)
                means.append(dl_df[col].mean())
                stds.append(dl_df[col].std())
    ...
    ax.set_ylabel("Mean Absolute Error (m)", fontsize=14, fontweight="bold", labelpad=8)
        ...
        if i > 0:
            ...
            if drop > 0:
                ...
                ax.annotate(
                    ...
                    arrowprops=dict(
                        arrowstyle="->", lw=1.8, color=PALETTE["deep_blue"]
                    ),
                )
                ax.text(
                    i - 0.45,
...
def plot_uncertainty_vs_error(
    ...
    ax.set_xlabel(
        "Uncertainty Quintile  (Q1 = lowest σ, Q5 = highest σ)",
        fontsize=13,
        fontweight="bold",
        labelpad=8,
    )
    ax.set_ylabel("Mean Absolute Error (m)", fontsize=13, fontweight="bold", labelpad=8)
    ax.set_title(
...
def plot_spatial_uncertainty_map(
    ...
    cbar.set_label("Absolute Error (m)", fontsize=13, fontweight="bold", labelpad=10)
    cbar.ax.tick_params(labelsize=11)

    ax.grid(True, linestyle=":", lw=0.5, color=PALETTE["grid"], alpha=0.9, zorder=1)
    for spine in ax.spines.values():
        spine.set_color(PALETTE["grid"])
        spine.set_linewidth(0.7)
    ...
    plt.setp(ax.xaxis.get_majorticklabels(), rotation=15, ha="right", fontsize=10)
    ...
    info = (
        f"Mean |error|: {df['abs_error'].mean():.2f} m\n"
...
# ════════════════════════════════════════════════════════════════════════
def plot_per_fold_results(csv_path="results/loso_summary.csv"):
    ...
    ax.set_xlabel("Fold (Held-out Sensor)", fontsize=13, fontweight="bold", labelpad=8)
